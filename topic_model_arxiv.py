...
class TopicModeler:
    ...
    def optimal_model_search(self, start=2, stop=5, step=3):
        ...
        for num_topics in range(start, stop, step):
            model = gensim.models.ldamodel.LdaModel(corpus=self.corpus, num_topics=num_topics, id2word=self.dictionary)
    ...
    def produce_doc_topic_summary_df(self):
        ...
        doc_topic_summary = pd.concat([sent_topics_df, contents], axis=1)

        doc_topic_summary = doc_topic_summary.reset_index()
        doc_topic_summary.columns = ['Document_No', 'Dominant_Topic', 'Topic_Perc_Contrib', 'Keywords', 'Text']

        return doc_topic_summary

    def produce_topic_summary_df(self, doc_topic_summary):
        # Number of Documents for Each Topic
        topic_counts = doc_topic_summary['Dominant_Topic'].value_counts()

        # Percentage of Documents for Each Topic
        topic_contribution = round(topic_counts/topic_counts.sum(), 4)

        # Topic Number and Keywords
        topic_num_keywords = doc_topic_summary[['Dominant_Topic', 'Topic_Keywords']]
